...
public class QmMath {
    ...
    public ArrayList<String> setImplicantArrayList(ArrayList<Implicant> implicantArrayList) {
        this.implicantArrayList = implicantArrayList;
        return simplifyMainFunction();
    }

    private ArrayList<String> simplifyMainFunction() {
        divideIntoGroups();
        ...
        for (int i = 0; i < finalArray.size(); i++) {
            ...
            strings.add(im.doubleTypeString);
        }
        /*System.out.println("化简结果为：");
        for (int i = 0; i < finalArray.size(); i++) {
            Implicant im = finalArray.get(i);
            if (!im.isNCI && i != finalArray.size() - 1) {
                System.out.print("!!" + im.doubleTypeString + "!!");
